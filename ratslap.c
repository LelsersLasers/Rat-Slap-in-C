...
int game(int *pile1, int *pile2) {
    ...
    while (1) {
		...
		int isFace = -1; // neg 1 means dont need to worry about it
        int inRound = 1;
        ...
        while (inRound) {
            ...
            if (getCardsInPile(currentPile) > 0) {
                ...
                if (isDouble || isSandwich) { // slap prio
					...
					int isLastCardFace = checkFace(card);
					if (isLastCardFace > 0) { // its a face card
						if (isFace == -1) { // not already in a face cycle
							printf("Player %i must draw a face in %i flips.\n", switchTurn(currentPlayer), isLastCardFace);
						}
						else {
							// end current and start new
							printf("Player %i has gotten a face card!\n", currentPlayer);
							printf("Player %i now must draw a face in %i flips.\n", switchTurn(currentPlayer), isLastCardFace);
						}
						isFace = isLastCardFace;
					}
					else { // last was not a face card
						if (isFace == -1) {
							// do nothing, we chilling
						}
						else if (isFace > 0) { // in active cycle
							isFace = isFace - 1;
							printf("Player %i now has %i chances to get a face card.\n", currentPlayer, isLastCardFace);
							dontSwitch = 1;
						}
						else if (isFace == 0) {
